...
function RankRow({ e, separator }: { e: RankEntry; separator?: boolean }) {
	return (
		<div
			className={`rank-row${e.isTarget ? " target" : ""}${separator ? " separator" : ""}`}
		>
	...
	return (
		<div className="section">
			<div className="section-title">
				📍 Top Rank รวม {regionName}{" "}
				<span className="text-red">(10 อันดับ)</span>
			</div>
			<CfBanner
				points={data.changfamilyPoints}
				label="Changfamily พิเศษที่คุณได้รับ"
			/>
			<div className="rank-grid">
				<div className="rank-grid-col">
					{left.map((e) => (
						<RankRow key={e.rank} e={e} />
					))}
				</div>
				<div className="rank-grid-col">
					{right.map((e) => (
...
function CategorySection({ cat }: { cat: CategoryResult }) {
	...
	const top5 = cat.entries.filter((e) => e.rank <= cat.limit);
	const storeExtra = cat.entries.find((e) => e.isTarget && e.rank > cat.limit);

	return (
		<div className="section">
			<div className="section-title">
				{icon} {cat.name} {cat.province}{" "}
				<span className="text-red">({cat.limit} อันดับ)</span>
			</div>
			<CfBanner
				points={cat.changfamilyPoints}
				label="Changfamily พิเศษที่คุณได้รับ"
			/>
			{top5.map((e) => (
				<RankRow key={e.rank} e={e} />
...
export default function Home() {
	...
	const doSearch = useCallback(
		async (searchCode: string, searchPhone: string) => {
			setStatus("loading");
			setResult(null);
			setErrorMsg("");
			try {
				const res = await fetch(
					`/api/search?code=${encodeURIComponent(searchCode)}&phone=${encodeURIComponent(searchPhone)}`,
				);
				...
				if (res.status === 429) {
					...
					retryRef.current = setTimeout(() => {
						clearInterval(tick);
						doSearch(searchCode, searchPhone);
					}, wait * 1000);
					return;
				}
				if (!res.ok) {
					...
					{status === "not_found" && (
						<>
							...
							<div className="alert alert-empty">
								...
								<p>
									ไม่พบข้อมูล รหัสร้าน <strong>"{code}"</strong>{" "}
									หรือเบอร์โทรศัพท์ไม่ถูกต้อง
								</p>
							</div>
						</>
					)}
